...
| **defaultWhenEmpty** | `Date` | `new Date()` | Represents the default date for the picker when the initial value is not set. This prop is parsed by Moment.js, so it is possible to use a date `string` or a `moment` object. |
